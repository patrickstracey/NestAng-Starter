...
export class UserService {
  ...
  async getUser(token: TokenInterface): Promise<UserInterface> {
    try {
      let account = (await this.db.findOne({ _id: this._dbService.bsonConvert(token.uid) })) as UserInterface;
  ...
  async insertNewUser(signupAttempt: SignupDto, encryptedPassword: string): Promise<UserInterface> {
    try {
      ...
      let returnedUser: UserInterface = { _id: insertResult["insertedId"], ...newUser };
  ...
  async updatePassword(account_id: string, newPassword: string): Promise<boolean> {
    try {
      ...
      const result = await this.db.findOneAndUpdate({ _id: this._dbService.bsonConvert(account_id) }, { $set: update }, options);
      return !!result;
    } catch (e) {
  ...
  private cleanUser(account: UserInterface): UserInterface {
